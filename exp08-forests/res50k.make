...
rf-test:
	for rank in family genus; do \
		...
		for i in {1..5}; do \
			{ time java -Xmx6000M weka.classifiers.meta.FilteredClassifier -no-predictions -l output/rf.$$rank.model -T output/res50k.$$rank.s1.arff > /dev/null; } 2>> $(RESULTS)/res50k.$$rank.rf.s1.testing.time; \
		done; \
	done
...
nb-test:
	for rank in family genus; do \
		...
		for i in {1..5}; do \
			{ time java -Xmx6000M weka.classifiers.meta.FilteredClassifier -no-predictions -l output/nb.$$rank.model -T output/res50k.$$rank.s1.arff > /dev/null; } 2>> $(RESULTS)/res50k.$$rank.nb.s1.testing.time; \
		done; \
	done
